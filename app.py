...
def main():
    ...
    with get_db() as db:
        ...
        # Plano de Ataque
            ...
            st.markdown("### ➕ Novo Giant")
            # Form para criar novo Giant
            with st.form("novo_giant"):
                col1, col2 = st.columns(2)
                with col1:
                    nome_giant = st.text_input("Nome do Giant", placeholder="Ex: Cartão Nubank")
                ...
                if st.form_submit_button("Criar Giant"):
                    if nome_giant and valor_total > 0:
                        ...
                        db.add(giant)
                        db.commit()
                        st.success("Giant criado com sucesso!")
                        st.rerun()
                    else:
